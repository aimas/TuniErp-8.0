...
In TuniERP, a workflow is a technical artefact to manage a set of "things to
...
- Activities define work that should be done within the TuniERP server, such
...
All in all, TuniERP's workflow system provides:
...
A company using TuniERP may want to add discount support to orders, where sales
...
TuniERP knows where to start the workflow traversal after it is instanciated.
...
for a record, TuniERP simply processes all of them, and evaluate all their
...
It is important for TuniERP to know when a workflow instance is completed. A
...
giving a signal name in the attribute ``signal_send``. TuniERP processes those
...
After an activity has been processed, TuniERP evaluates its transition to reach
the next activity in the flow.

However if an activity has more than one transition, TuniERP must decide which
...
``XOR`` (default)
    By default, TuniERP will use the first transition (in ``sequence`` order)
...
``OR``
    ...
    become valid later.
``AND``
    In ``AND`` mode, TuniERP will wait until *all* transitions are satisfied, and
...
signal or trigger is defined), the condition is evaluated by TuniERP, and if
...
defined (in addition to the TuniERP ``safe_eval`` environment):
