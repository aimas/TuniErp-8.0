...
This guide is about creating modules for TuniERP's web client.

To create websites with TuniERP, see :doc:`website`; to add business capabilities
or extend existing business systems of TuniERP, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed TuniERP <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple TuniERP module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/tunierp/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to TuniERP's
:option:`addons path <tunierp.py --addons-path>`, create a new database and
...
Files used in the "web" side of an TuniERP module must be placed in a ``static``
...
.. warning::
    ...
    "developer mode": log into your TuniERP instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    TuniERP screen) and select :guilabel:`About TuniERP` then :guilabel:`Activate
    the developer mode`:

    .. image:: web/about_tunierp.png
        :align: center
...
The TuniERP framework uses one such pattern to define modules within web addons,
...
In TuniERP web, modules are declared as functions set on the global ``openerp``
...
* the first parameter is the current instance of the TuniERP web client, it gives
  access to various capabilities defined by the TuniERP (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the TuniERP web client needs to call them
...
For simplicity and developer-friendliness TuniERP web provides a class
...
The TuniERP web client bundles jQuery_ for easy DOM manipulation. It is useful
...
TuniERP Web makes specific components responsible for sections of a page. In
TuniERP web, the base for such components is the :class:`~openerp.Widget`
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear TuniERP user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in TuniERP Web, you will need to reload
    the page. There is no need to restart the TuniERP server.

The ``HomePage`` widget is used by TuniERP Web and managed automatically.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear TuniERP user!</div>
...
As many other environments, TuniERP's solution is to use a `template engine`_.
TuniERP's template engine is called :ref:`reference/qweb`.
...
* It has special support in TuniERP Web's :class:`~openerp.Widget`, though it
  can be used outside of TuniERP's web client (and it's possible to use
...
.. note::
    ...
    like TuniERP :ref:`views <reference/views>`.
    ...
    characteristic of TuniERP, and losing it was considered unacceptable.
...
The class system of the TuniERP web framework allows direct modification of
...
In TuniERP, translations files are automatically generated by scanning the source
...
Communication with the TuniERP Server
==================================

Contacting Models
-----------------

Most operations with TuniERP involve communicating with *models* implementing
...
communicating with TuniERP requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, TuniERP web provides
...
The class used to call TuniERP models is :class:`openerp.Model`. It is
instantiated with the TuniERP model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~openerp.web.Model.call` can be used to call any (public) method of an
TuniERP model. It takes the following positional arguments:
...
the time zone of the user, used to compute correctly dates and times if TuniERP
...
are related to the configuration of the current user in TuniERP plus the
...
While :func:`~openerp.Model.call` is sufficient for any interaction with TuniERP
models, TuniERP Web provides a helper for simpler and clearer querying of models
...
.. exercise:: Pet Toys List
    ...
    In TuniERP, images are generally stored in regular fields encoded as
...
In TuniERP, many operations start from an :ref:`action <reference/actions>`:
...
In TuniERP Web, the component responsible for handling and reacting to these
...
the TuniERP web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for TuniERP web. The server simply sends
...
from the TuniERP web client root.
...
Much of TuniERP web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`TuniERP views <reference/views>` are implemented through a subclass
...
The *search view* is considered a view type by the main TuniERP framework, but
...
* It must correctly implement the 3 field attributes available in all fields
  of TuniERP. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in TuniERP have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the TuniERP server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in TuniERP. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the TuniERP web
client.
...
user to modify it can be useful, but most fields in TuniERP also allow editing.
