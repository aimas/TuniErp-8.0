...
.. warning::
    ...
    * This guide assumes :ref:`an installed TuniERP <setup/install>`

Creating a basic module
=======================

In TuniERP, tasks are performed by creating modules.

Modules customize the behavior of an TuniERP installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`TuniERP's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./tunierp.py scaffold Academy my-modules
...
* start the TuniERP server

  .. code-block:: console

      $ ./tunierp.py --addons-path addons,my-modules
...
display logic. TuniERP allows any Python templating system, but provides its
...
Finally restart TuniERP and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Local Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, TuniERP can be restarted :option:`and update modules at
    the same time<tunierp.py -u>`:

    .. code-block:: console

        $ tunierp.py --addons-path addons,my-modules -d academy -u academy

Going to http://localhost:8069/academy/academy/ should now result in:

.. image:: website/basic-list.png

Storing data in TuniERP
====================

:ref:`TuniERP models <reference/orm/model>` map to database tables.
...
TuniERP bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but TuniERP 8 added deeper
...
You're now in TuniERP "proper": the administrative interface. For now click on
...
restart TuniERP, access http://localhost:8069/academy/Alice/ and
...
Restart TuniERP, access http://localhost:8069/academy/2, note how the old value
...
TuniERP provides an additional converter called ``model`` which provides records
...
Restart TuniERP and upgrade the module, then you can visit each teacher's page.
...
Restart TuniERP and update the views, reload the teacher's page and… the field
...
Restart TuniERP and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the TuniERP administration
--------------------------------------------------------------

The TuniERP administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the TuniERP backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which TuniERP should
   execute or data display. Data display actions are called *window actions*,
   and tell TuniERP to display a given *model* according to a set of views…
...
Editing in the TuniERP administration
----------------------------------

By default, an TuniERP model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) TuniERP will automatically create a basic one
...
TuniERP provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of TuniERP's email and messaging
...
TuniERP also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the TuniERP system. We can easily make
...
restart TuniERP, update your module, there is now a :guilabel:`Shop` section in
