...
class base_report(report_sxw.rml_parse):
    ...
    def _load(self, name, form):
        ...
        self.cr.execute("SELECT l10n_tu_line.code,definition FROM l10n_tu_line LEFT JOIN l10n_tu_report ON l10n_tu_report.id=report_id WHERE l10n_tu_report.code=%s",(name,))
    ...
    def _load_accounts(self, form, code, definition, fiscalyear, period_ids):
        ...
        if fiscalyear.state != 'done' or not code.startswith('bpcheck'):
            ...
            if fiscalyear.state == 'done':
                closed_cond=" AND (aml.move_id NOT IN (SELECT account_move.id as move_id FROM account_move WHERE period_id = ANY(%s) AND journal_id=(SELECT res_id FROM ir_model_data WHERE name='closing_journal' AND module='l10n_tu')) OR (aa.type != 'income' AND aa.type !='expense'))"
                ...
                for account in accounts:
                    if(line["code"].startswith(account)):
                        ...
                        if(type == "S"):
                            ...
                            value=line["debit"]-line["credit"]
                            if(abs(value)<0.001): value=0.000
                        elif(type == "C"):
